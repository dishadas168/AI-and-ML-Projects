...
def alphabeta(player, game, time_left, depth, alpha=float("-inf"), beta=float("inf"), my_turn=True):
    ...
    if depth==0:
        ...
        #print(available_moves)
        for move in available_moves:
            val = alphabeta(game.get_inactive_player(), game.forecast_move(move)[0], time_left, depth-1,alpha, beta, False)
            if val[1]>bestVal:
                bestVal = val[1]
                bestMove = move
            if bestVal > beta:
        ...
        bestMove = (-1,-1,False)
        for move in available_moves:
            val = alphabeta(game.get_inactive_player(), game.forecast_move(move)[0], time_left, depth-1,alpha,beta, True)
            if val[1]<bestVal:
                bestVal = val[1]
                bestMove = move
            if bestVal < alpha:
